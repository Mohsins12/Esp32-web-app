...
async function fetchLatestData() {
  try {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/${encodeURIComponent(TABLE_NAME)}?select=accel_x,accel_y,accel_z,gyro_x,gyro_y,gyro_z,angle_magnitude,posture_status&order=id.desc&limit=1`,
      {
        headers: {
          apikey: SUPABASE_KEY,
